...
(function (platui) {
    ...
    var Modal = (function (_super) {
        ...
        Modal.prototype._alignModal = function (ev) {
            var _this = this;
            var utils = this.utils, isNull = utils.isNull, _document = this._document, documentEl = _document.documentElement, scrollEl = isNull(documentEl) || !documentEl.scrollHeight ? _document.body : documentEl;
    ...
    var Input = (function (_super) {
        ...
        Input.prototype._initializeType = function () {
            var type = this._type, event = __$tap, actionElement = this._actionElement, _Exception;
            switch (type) {
                case 'text':
                ...
                case 'radio':
                    _Exception = this._Exception;
                    _Exception.warn(type + ' is not supported by ' + this.type + '. Please use a ' + __Radio + ' instead.', _Exception.CONTROL);
                    break;
                case 'checkbox':
                    _Exception = this._Exception;
                    _Exception.warn(type + ' is not supported by ' + this.type + '. Please use a ' + __Checkbox + ' instead.', _Exception.CONTROL);
                    break;
                case 'range':
                    _Exception = this._Exception;
                    _Exception.warn(type + ' is not supported by ' + this.type + '. Please use a ' + __Slider + ' instead.', _Exception.CONTROL);
                    break;
                default:
                    _Exception = this._Exception;
                    _Exception.warn(type + ' is not yet fully supported by ' + this.type + '. Defaulting to type="text".', _Exception.CONTROL);
                    type = 'text';
                    this._pattern = this._pattern || /[\S\s]*/;
                    this._actionHandler = this._checkText.bind(this);
                    this._typeHandler = this._erase;
    ...
    var Carousel = (function (_super) {
        ...
        Carousel.prototype.loaded = function () {
            var _this = this;
            var _utils = this.utils, context = this.context;
        ...
        Carousel.prototype.goToNext = function () {
            ...
            var animationOptions = {}, length = this._getLength();
            if (!length) {
                return null;
            }
        ...
        Carousel.prototype._verifyLength = function () {
            var _this = this;
            var context = this.context, _utils = this.utils;
            ...
            var maxIndex = context.length - 1, offset = this._getLength(), maxOffset = maxIndex * offset;
            if (!offset) {
                this._checkArrows();
                return;
            }
        ...
        Carousel.prototype._goToIndex = function (index) {
            ...
            var animationOptions = {}, interval = (oldIndex - index) * this._getLength();
            if (!interval) {
                return null;
            }
        ...
        Carousel.prototype._infiniteNext = function () {
            var _this = this;
            var index = this._index, offset = -this._getLength(), animationOptions = {};
            if (!offset) {
                return null;
            }
        ...
        Carousel.prototype._infinitePrevious = function () {
            var _this = this;
            var index = this._index, animationOptions = {}, maxLength = this.context.length, offset = this._getLength(), maxOffset = maxLength * offset;
            if (!offset) {
                return null;
            }
            animationOptions[this._transform] = this._calculateStaticTranslation(offset);
        ...
        Carousel.prototype._init = function () {
            var _this = this;
            var foreach = this.controls[0], container = this._container, itemsLoaded = function () {
        ...
        Carousel.prototype._initializeInfinite = function () {
            ...
            this._slider.style[this._transform] = this._calculateStaticTranslation(-length);
        ...
        Carousel.prototype._calculateStaticTranslation = function (interval) {
            return this._isVertical ? 'translate3d(0,' + (this._currentOffset += interval) + 'px,0)' : 'translate3d(' + (this._currentOffset += interval) + 'px,0,0)';
        };
        /**
        ...
        Carousel.prototype._calculateDynamicTranslation = function (ev) {
            return this._isVertical ? 'translate3d(0,' + (this._currentOffset + (ev.clientY - this._lastTouch.y)) + 'px,0)' : 'translate3d(' + (this._currentOffset + (ev.clientX - this._lastTouch.x)) + 'px,0,0)';
        };
        /**
         * Obtains the current browser's transform property value.
         */
        Carousel.prototype._setTransform = function () {
            var style = this._slider.style, isUndefined = this.utils.isUndefined;
         ...
         */
        Carousel.prototype._setPosition = function () {
    ...
    var Listview = (function (_super) {
        ...
        Listview.prototype.loaded = function () {
            var options = this.options.value, _utils = this.utils, isString = _utils.isString, element = this.element, viewport = this._viewport = element.firstElementChild, scrollContainer = this._scrollContainer = viewport.firstElementChild, loading = this._loading = options.loading, animate = this._animate = options.animate === true, requestItems = options.onItemsRequested, refresh = options.onRefresh, itemTemplate = options.itemTemplate, _Exception;
        ...
        Listview.prototype._setAliases = function () {
            var aliases = this.options.value.aliases, _utils = this.utils;
        ...
        Listview.prototype._determineTemplates = function (itemTemplate, itemTemplateKey, groupHeaderTemplate) {
            var _Exception, _utils = this.utils, bindableTemplates = this.bindableTemplates, templates = this._templates, template;
        ...
        Listview.prototype._createGroupTemplate = function () {
            var _this = this;
            var _document = this._document, _utils = this.utils, options = this.options.value, bindableTemplates = this.bindableTemplates, groupHeaderTemplate = this._groupHeaderTemplate, groupHeader = this._templates[groupHeaderTemplate], listviewGroup = __Listview + '-group', group = _document.createElement('div'), itemContainer = _document.createElement('div'), headerPromise;
        ...
        Listview.prototype._addGroups = function (numberOfGroups, index, animateItems) {
            var _this = this;
            var _utils = this.utils, context = this.context, initialIndex = index, max = +(index + numberOfGroups);
        ...
        Listview.prototype._createItems = function (index, count, group, animateItems) {
            var _this = this;
            var _utils = this.utils, opGroup = group || this._defaultGroup, control = opGroup.control;
        ...
        Listview.prototype._renderUsingFunction = function (index, group, animate) {
            var _this = this;
            var _Promise = this._Promise, _utils = this.utils, opGroup = group || this._defaultGroup, control = opGroup.control, identifier, context;
            if (this === control) {
                identifier = index;
                context = this.context;
            }
        ...
        Listview.prototype._handleScroll = function () {
            ...
            var scrollContainer = this._scrollContainer, scrollLength = 0.8 * (this._isVertical ? scrollContainer.scrollHeight : scrollContainer.scrollWidth), _utils = this.utils;
            if (scrollLength === 0) {
                return;
            }
        ...
        Listview.prototype._executeChildEvent = function (groupName, changes) {
            var _utils = this.utils, method = '_' + changes[0].type;
        ...
        Listview.prototype._splice = function (changes, group) {
            var _this = this;
            var _utils = this.utils, change = changes[0], opGroup = group || this._defaultGroup, addCount = change.addedCount, control = opGroup.control, addQueue = opGroup.addQueue, animating = this._animate;
